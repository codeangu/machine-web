...
export class CustomerLedgerComponent implements OnInit {
  ...
  ngOnInit() { this.fetchLedger(); }

  fetchLedger() {
    this.loading = true;
    this.service.getById<any>('customer/ledger', this.customerId).subscribe({
  ...
  submitTransaction() {
    ...
    this.http.post(`${environment.baseURL}/customer/payment`, {
      customerId: this.customerId,
      amount: this.txAmount,
      transactionType: this.txType,
      note: `${this.txMethod} — ${this.txNote || ''}`.trim().replace(/— $/, ''),
      date: this.txDate
    }, { headers }).subscribe({
      next: () => {
      ...
      error: (err) => {
        this.txLoading = false;
        this.txError = err.error?.msg || 'Error recording transaction';
      }
    });
  }
  ...
  private buildPrintTable(): string {
    ...
    const rows = (d.ledger || []).map((item: any, i: number) => `
      <tr>
        <td>${i + 1}</td>
        <td>${new Date(item.date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: '2-digit' })}</td>
        <td>${item.transactionType || ''}${item.description ? ' — ' + item.description : ''}</td>
        <td style="text-align:right;color:#e11d48">${item.debit > 0 ? 'Rs. ' + item.debit.toLocaleString() : '—'}</td>
        <td style="text-align:right;color:#16a34a">${item.credit > 0 ? 'Rs. ' + item.credit.toLocaleString() : '—'}</td>
        <td style="text-align:right;font-weight:700">Rs. ${(item.runningBalance || 0).toLocaleString()}</td>
      </tr>`).join('');
    return `
      <table style="width:100%;border-collapse:collapse;font-size:12px">
        <thead>
          <tr style="background:#e2e8f0;text-align:left">
            <th style="padding:6px">#</th><th style="padding:6px">Date</th><th style="padding:6px">Type &amp; Description</th>
            <th style="padding:6px;text-align:right">Debit (Sale)</th><th style="padding:6px;text-align:right">Credit (Paid)</th>
        ...
        <tfoot>
          <tr style="background:#1e293b;color:#fff;font-weight:700">
            ...
            <td style="padding:6px;text-align:right">Rs. ${(d.totalDebit || 0).toLocaleString()}</td>
            <td style="padding:6px;text-align:right">Rs. ${(d.totalCredit || 0).toLocaleString()}</td>
            <td style="padding:6px;text-align:right">Rs. ${(d.currentBalance || 0).toLocaleString()}</td>
          </tr>
        </tfoot>
      </table>`;
  }
  ...
  printLedger() {
    ...
    win.document.write(`
      <html><head><title>Customer Ledger - ${this.customerName}</title>
      ...
      <style>
        ...
        <div class="brand">
          ...
          <div style="text-align:right">
            <div style="font-weight:700;font-size:15px">Customer Account Statement</div>
            <div style="color:#64748b">Generated: ${today}</div>
          </div>
        </div>
        <div><b style="font-size:15px">${this.customerName}</b></div>
        <div class="sum">
          <div><small>Total Sales</small><b>Rs. ${(d.totalDebit||0).toLocaleString()}</b></div>
          <div><small>Total Received</small><b style="color:#16a34a">Rs. ${(d.totalCredit||0).toLocaleString()}</b></div>
          <div><small>Balance Due</small><b style="color:#e11d48">Rs. ${(d.currentBalance||0).toLocaleString()}</b></div>
        </div>
        ${content}
      </body></html>`);
    win.document.close();
