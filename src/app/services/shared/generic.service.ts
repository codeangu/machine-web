...
export class GenericService {
  ...
  constructor( private http: HttpClient,
    ...
    getPdfInvoice(type:any,id: string): Observable<any> {
      return this.http
        .get(this.baseURL + type+ '/invoice/.pdf?id='+ id,{ responseType: 'blob' })
        .pipe(catchError(this.processHTTPMsgService.handleError));
    }
    ...
    // getPaginatedWithQuery(type:any,pageSize: any,page:any,paramsObj): Observable<any> {
    //   let params = '';
    //   if(paramsObj && Object.keys(paramsObj).length>0){
    //     params='?'+this.objdynamicParams.dynamicParams(paramsObj);
    //  }
    //   return this.http
    //   .get<any[]>(this.baseURL + type +'/' + pageSize + '/' + page +params)
    //   .pipe(catchError(this.processHTTPMsgService.handleError));
    // }
  
    getPaginated(type:any,pageSize: any,page:any): Observable<any> {
      return this.http
      .get<any[]>(this.baseURL + type +'/' + pageSize + '/' + page)
      .pipe(catchError(this.processHTTPMsgService.handleError));
    }
    // getAllWithQuery(type:string,paramsObj?:any) { 
    //   let params = '';
    //   if(paramsObj && Object.keys(paramsObj).length>0){
    //     params='?'+this.objdynamicParams.dynamicParams(paramsObj);
    ...
    //   return this.http
    //     .get<any[]>(this.baseURL + type+params)
    //     .pipe(catchError(this.processHTTPMsgService.handleError));
    // }
    ...
    deleteAllByUserId(type: any, userId: string): Observable<any> {
      console.log('Deleting all records for userId:', userId);
      return this.http
        .delete(this.baseURL + type + '?userId=' + userId)
